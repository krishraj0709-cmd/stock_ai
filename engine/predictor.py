...
class PredictionEngine:
    ...
    def predict(self, symbol, mode="longterm"):
        ...
        return {
            "symbol": symbol.upper(), "mode": mode, "signal": signal,
            "confidence": round(confidence * 100, 1),
            "entry_price": round(entry_price, 2),
            "target": round(target, 2), "stop_loss": round(stop_loss, 2),
            "expected_pct": round((target / entry_price - 1) * 100, 2),
            "risk_level": self._risk_level(atr, entry_price),
            "probabilities": {"BUY": round(probs[2]*100,1),
    ...
    def _intraday_signal(self, df):
        ...
        return SIGNAL_MAP[idx], float(probs[idx]), probs.tolist()
    ...
    def _risk_level(self, atr, price):
        ...
        if ratio > 0.025: return "HIGH"
        if ratio > 0.012: return "MEDIUM"
        return "LOW"
