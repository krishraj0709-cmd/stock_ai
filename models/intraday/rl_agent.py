...
class TradingEnv(gym.Env):
    ...
    def __init__(self, df, feature_cols, initial_capital=100000.0, transaction_cost=0.0003):
        super().__init__()
        self.df = df.reset_index(drop=True)
        self.feature_cols = feature_cols
        self.initial_capital = initial_capital
        self.transaction_cost = transaction_cost
        self.n_feats = len(feature_cols) + 2
        self.observation_space = gym.spaces.Box(low=-10.0, high=10.0, shape=(self.n_feats,), dtype=np.float32)
    ...
    def _obs(self):
        ...
        extra = np.array([self.position / 100.0, self.cash / self.initial_capital], dtype=np.float32)
    ...
    def step(self, action):
        ...
        return self._obs(), reward, done, False, {"portfolio_value": curr_val}

    def render(self):
        pass

def train_rl_agent(df, feature_cols, timesteps=200000, save_path="models/intraday/ppo_trader"):
    def make_env():
    ...
    eval_cb = EvalCallback(eval_env, best_model_save_path=save_path,
                           eval_freq=10000, deterministic=True, render=False, verbose=0)
