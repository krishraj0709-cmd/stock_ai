...
def create_labels(df, horizon=20):
    ...
    df = df.copy()
    df["label"] = pd.cut(fwd, bins=[-np.inf, -0.05, 0.05, np.inf], labels=[0, 1, 2]).astype("Int64")
    return df.dropna(subset=["label"])

def train_xgboost(df, horizon=20, save_path="models/longterm/xgb_model.json"):
    ...
    print(classification_report(y, oof_preds.argmax(axis=1), target_names=["SELL", "HOLD", "BUY"]))
...
def load_xgboost(path="models/longterm/xgb_model.json"):
