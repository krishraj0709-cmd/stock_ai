...
# ════════════════════════════════════════════════════════════════════════════════
with tab1:
    section_title("Predict Signal")
    s_col, b_col = st.columns([3, 1])
    with s_col:
        search_symbol = st.text_input(
            "",
            value=symbol,
            placeholder="Type stock symbol — RELIANCE, TCS, INFY, ZOMATO...",
...
# ════════════════════════════════════════════════════════════════════════════════
with tab2:
    section_title("High Return Scanner")

    scan_col1, scan_col2 = st.columns([3, 1])
    with scan_col1:
        st.markdown("""<div style="font-size:13px;color:#5a6478;font-family:'JetBrains Mono',monospace">
        Scans NSE stocks for high-probability setups using ensemble model + market regime filter</div>""",
        unsafe_allow_html=True)
    with scan_col2:
        run_scan = st.button("⚡  Run Full Scan", use_container_width=True)

    if run_scan:
        with st.spinner("Scanning markets — applying ensemble filters…"):
            try:
                resp = requests.get(f"{API_URL}/scan", timeout=120)
                scan_data = resp.json().get("results", [])
            except Exception as e:
                st.warning(f"Scanner endpoint not ready: {e}")
                scan_data = []

        if scan_data:
            section_title(f"{len(scan_data)} opportunities found")
            for item in scan_data:
                sig = item.get("signal","HOLD")
                sig_text = {"BUY":"#00f5a0","SELL":"#ff3b6b","HOLD":"#ffb830"}.get(sig,"#8892a4")
                conf = item.get("confidence", 50)
                c1, c2, c3, c4, c5, c6 = st.columns([2,1.2,1.5,1.5,1.5,1.5])
                c1.markdown(f"**{item.get('symbol','--')}**")
                c2.markdown(signal_pill(sig), unsafe_allow_html=True)
                c3.markdown(f"<span style='font-family:JetBrains Mono,monospace;color:{sig_text};font-size:13px'>{conf}%</span>", unsafe_allow_html=True)
                c4.markdown(f"<span style='font-family:JetBrains Mono,monospace;font-size:13px'>₹{item.get('entry_price','--')}</span>", unsafe_allow_html=True)
                c5.markdown(f"<span style='font-family:JetBrains Mono,monospace;font-size:13px;color:#00f5a0'>₹{item.get('target','--')}</span>", unsafe_allow_html=True)
                c6.markdown(f"<span style='font-family:JetBrains Mono,monospace;font-size:13px;color:#ff3b6b'>₹{item.get('stop_loss','--')}</span>", unsafe_allow_html=True)
                st.divider()
        else:
            st.info("No scan results yet — run the scanner or connect the /scan endpoint.")
